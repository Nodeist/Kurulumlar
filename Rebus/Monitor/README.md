...
# Rebus Grafana Monitor Kurulum Rehberi
## Önkoşullar
...
| ANAHTAR |DEĞER |
|---------------|-------------|
| **bond_denom** | Denom Degeri. Örneğin Rebus için `arebus` |
| **bench_prefix** | Bench Prefix Değeri. Örneğin Rebus için `rebus`. Bu değeri cüzdan adresinizden öğrenebilirsiniz. **rebus**jcqum902je9zwevat7zqczskwd4lhwuj9vwgsu |
| **grpc_port** | "app.toml" dosyasında tanımlanan doğrulayıcı "grpc" bağlantı noktanız. Varsayılan değer `38090`dır |
| **rpc_port** | "config.toml" dosyasında tanımlanan doğrulayıcı "rpc" bağlantı noktanız. Varsayılan değer `38657`dir |
