...
# Uptick Grafana Monitor Kurulum Rehberi
## Önkoşullar
...
| ANAHTAR |DEĞER |
|---------------|-------------|
| **bond_denom** | Denom Degeri. Örneğin Uptick için `auptick` |
| **bench_prefix** | Bench Prefix Değeri. Örneğin Uptick için `uptick`. Bu değeri cüzdan adresinizden öğrenebilirsiniz. **uptick**1r5g0kes6jutsydez9qw2tx6vuc8scpxn5qtyle |
...
** Eğer node kurulumunu bizim dökümanımızdan yaptıysanız, Kurulumlar sayfamızdan Uptick port adresini kontrol edebilirsiniz. **

![nodeist](https://i.hizliresim.com/8nedatw.png)

Bu örnekte resimde gördüğünüz gibi Uptick portumuz `40`.

Bunun anlamı şudur: Sizin default hali `9090` olan `adresport` unuz eğer node kurulumunu bizim dökümanımızdan yaptıysanız `40090`dır.

Aynı şekilde default hali `266657` olan `ladrport` unuz ise `40657`dir.
...
> Örneğin: ```$HOME/Nodeistor/ag_ekle.sh 1.2.3.4 Uptickvaloper1s9rtstp8amx9vgsekhf3rk4rdr7qvg8dlxuy8v Uptick1s9rtstp8amx9vgsekhf3rk4rdr7qvg8d6jg3tl Uptick```
