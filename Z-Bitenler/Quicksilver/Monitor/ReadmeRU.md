...
# Quicksilver Руководство по установке монитора Grafana
## Предпосылки
...
| КЛЮЧ | ЗНАЧЕНИЕ |
|----------|--------------|
| **bench_denom** | Номинальная стоимость. Например, `uqck` для Quicksilver |
| **bench_prefix** | Значение префикса скамейки. Например `quick` для Quicksilver. Узнать это значение можно по адресу вашего кошелька. **quick**jcqum902je9zwevat7zqczskwd4lhwuj9vwgsu |
| **adresport** | address порт. По умолчанию 9090. проверить app.toml |
| **ladrport** | ladr Порт. По умолчанию 26657. Проверьте в config.toml. |

** Если вы установили наш узел из нашего документа, вы можете проверить адрес порта «Quicksilver» на нашей странице установки. **

![nodeist](https://i.hizliresim.com/8nedatw.png)

В этом примере наш порт Quicksilver — «36», как вы можете видеть на рисунке.

Это означает: Ваш «адрес», который по умолчанию равен «9090», будет «36090», если вы установили узел из нашего документа.

Точно так же ваш «ladrport», который по умолчанию равен «26657», равен «36657».
