...
# Realio Grafana Monitor Installation Guide
## Prerequisites
...
| KEY |VALUE |
|----------|-------------|
| **bond_denom** | Denom Value. For example `ario` for Realio |
| **bench_prefix** | Bench Prefix Value. For example `realio` for Realio. You can learn this value from your wallet address. **realio**jcqum902je9zwevat7zqczskwd4lhwuj9vwgsu |
| **grpc_port** | Your validator `grpc` port that is defined in `app.toml` file. Default value is `15090` |
| **rpc_port** | Your validator `rpc` port that is defined in `config.toml` file. Default value is `15657` |
