...
# Loyal Grafana Monitor Installation Guide
## Prerequisites
...
| KEY |VALUE |
|----------|-------------|
| **bond_denom** | Denom Value. For example `lyl` for Loyal |
| **bench_prefix** | Bench Prefix Value. For example `loyal` for Loyal. You can learn this value from your wallet address. **loyal**jcqum902je9zwevat7zqczskwd4lhwuj9vwgsu |
| **grpc_port** | Your validator `grpc` port that is defined in `app.toml` file. Default value is `56090` |
| **rpc_port** | Your validator `rpc` port that is defined in `config.toml` file. Default value is `56657` |
