...
# SGE Grafana Monitor Installation Guide
## Prerequisites
...
| KEY |VALUE |
|----------|-------------|
| **bond_denom** | Denom Value. For example `usge` for SGE |
| **bench_prefix** | Bench Prefix Value. For example `sge` for SGE. You can learn this value from your wallet address. **sge**jcqum902je9zwevat7zqczskwd4lhwuj9vwgsu |
| **grpc_port** | Your validator `grpc` port that is defined in `app.toml` file. Default value is `51090` |
| **rpc_port** | Your validator `rpc` port that is defined in `config.toml` file. Default value is `51657` |
