...
# Androma Grafana Monitor Kurulum Rehberi
## Önkoşullar
...
| ANAHTAR |DEĞER |
|---------------|-------------|
| **bond_denom** | Denom Degeri. Örneğin Androma için `uandr` |
| **bench_prefix** | Bench Prefix Değeri. Örneğin Androma için `andr`. Bu değeri cüzdan adresinizden öğrenebilirsiniz. **andr**1r5g0kes6jutsydez9qw2tx6vuc8scpxn5qtyle |
| **grpc_port** | "app.toml" dosyasında tanımlanan doğrulayıcı "grpc" bağlantı noktanız. Varsayılan değer `13090`dır |
| **rpc_port** | "config.toml" dosyasında tanımlanan doğrulayıcı "rpc" bağlantı noktanız. Varsayılan değer `13657`dir |
