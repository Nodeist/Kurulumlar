...
# Androma Grafana Monitor Installation Guide
## Prerequisites
...
| KEY |VALUE |
|----------|-------------|
| **bond_denom** | Denom Value. For example `uandr` for Androma |
| **bench_prefix** | Bench Prefix Value. For example `andr` for Androma. You can learn this value from your wallet address. **andr**jcqum902je9zwevat7zqczskwd4lhwuj9vwgsu |
| **grpc_port** | Your validator `grpc` port that is defined in `app.toml` file. Default value is `13090` |
| **rpc_port** | Your validator `rpc` port that is defined in `config.toml` file. Default value is `13657` |
