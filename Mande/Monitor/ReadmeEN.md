...
# Mande Grafana Monitor Installation Guide
## Prerequisites
...
| KEY |VALUE |
|----------|-------------|
| **bond_denom** | Denom Value. For example `mand` for Mande |
| **bench_prefix** | Bench Prefix Value. For example `mande` for Mande. You can learn this value from your wallet address. **mande**jcqum902je9zwevat7zqczskwd4lhwuj9vwgsu |
| **grpc_port** | Your validator `grpc` port that is defined in `app.toml` file. Default value is `46090` |
| **rpc_port** | Your validator `rpc` port that is defined in `config.toml` file. Default value is `46657` |
